...
```cron
0 6 * * * cd /home/hiroki/work/knowledge/tech/papers/paperpile-briefer && scripts/run_daily.sh
```

systemd user timerを使う場合:
...
- 要約生成は`codex exec`を使います。cron/systemdから実行しても同じ`codex`認証が使える状態にしてください。
